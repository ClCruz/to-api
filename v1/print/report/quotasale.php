<?php
   ...
   function get($comission, $dtinit, $dtend,$id_quotapartner) {
      $query = "EXEC pr_report_quotasale ?,?,?,?";
      // die("dd".json_encode($codVenda));
      $params = array($comission,$dtinit,$dtend,$id_quotapartner);
   ...
   $obj = get($_REQUEST["amount"], $_REQUEST["init"], $_REQUEST["end"], $_REQUEST["id_quotapartner"]);
   // die(json_encode($obj));
// die(get_uniquename_by_apikey(''));
...
<div style="text-align: center;font-size:12px; width:100%;">
   ...
   <p style="font-weight: bold; text-transform: uppercase">Relatório de vendas da API</p>
</div>
</htmlpageheader>
...
<table class="grid" style="padding-top:130px;">
   ...
   <table class="grid" repeat_header="1">
         ...
         <?php foreach ($obj as &$row) {?>
            <tr class="content">
               <td class="printonly_lines_values left" style="font-size:10px;"><?php echo $row["CodVenda"] ?></td>
               <td class="printonly_lines_values left"><?php echo $row["NomPeca"] ?>
               <br />
               <span style="font-size:10px;">
               <?php echo $row["DatApresentacao"] ?> - <?php echo $row["HorSessao"] ?>
               </span>
               </td>
               <td class="printonly_lines_values left"><?php echo $row["DatVenda"] ?></td>
               <td class="printonly_lines_values left"><?php echo $row["TipForPagto"] ?></td>
