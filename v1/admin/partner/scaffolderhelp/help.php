...
function getinfofromdb($id, $uniquename)
{
    ...
    foreach ($result as &$row) {
        $json = array(
            "id" => $row["id"], "name" => $row["name"], "uniquename" => $row["uniquename"], "domain" => $row["domain"], "databaseOK" => $row["databaseOK"], "userOK" => $row["userOK"], "databaseStatus" => $row["databaseStatus"], "userStatus" => $row["userStatus"], "json_ga" => $row["json_ga"], "json_meta_description" => $row["json_meta_description"], "json_meta_keywords" => $row["json_meta_keywords"], "json_template" => $row["json_template"], "json_info_title" => $row["json_info_title"], "json_info_description" => $row["json_info_description"], "json_info_cnpj" => $row["json_info_cnpj"], "json_info_companyaddress" => $row["json_info_companyaddress"], "json_info_companyname" => $row["json_info_companyname"], "scss_colors_primary" => $row["scss_colors_primary"], "scss_colors_secondary" => $row["scss_colors_secondary"], "scss_colors_text" => $row["scss_colors_text"], "scss_image_background" => $row["scss_image_background"], "apikey" => $row["apikey"], "fb_appid" => $row["fb_appid"], "videos" => array("list" => array())
        );
    }
    ...
    foreach ($result as &$row) {
        $aux = array(
            "order" => $row["fileorder"], "type" => $row["filetype"], "src" => $row["source"]
        );
    }
...
function getReplacement($db, $logoext)
{
    ...
    $ret[] = array("from" => "__wl-gateway-pagarme-api__", "to" => "ak_live_nKbw5PUnrPNcdxs2r9dzdL56C9cFwe");
    $ret[] = array("from" => "__wl-gateway-pagarme-cryptkey__", "to" => "ek_live_ssZt5NaGzGY4th5nR1LiTOOkjWChOn");
    return $ret;
}
function getDirContents($replace, $dir, &$results = array())
{
    ...
    foreach ($files as $key => $value) {
        ...
        if (is_file($path)) {
            ...
            $results[] = array(
                "name" => $path, "newname" => $file_newname, "isdir" => false, "hasreplace" => $file_hasreplace, "from" => $file_fromreplace, "to" => $file_toreplace
            );
        }
        if (is_dir($path)) {
            ...
            $results[] = array(
                "name" => $path, "newname" => $dir_newname, "isdir" => true, "hasreplace" => $dir_hasreplace, "from" => $dir_fromreplace, "to" => $dir_toreplace
            );
